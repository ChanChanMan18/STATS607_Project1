...
test_that("works for both gaussian and t distributions", {
  ...
  mse_gauss <- proper_cv_2fold(sample_gauss, p = ncol(sample_gauss$X), K = 3,
                                 sample_size = nrow(sample_gauss$X), seed = 1)
  mse_t <- proper_cv_2fold(sample_t, p = ncol(sample_t$X), K = 3,
...
test_that("Test K edge cases", {
  ...
  mse1 <- proper_cv_2fold(dat, p = ncol(dat$X), K = 1,
                          sample_size = nrow(dat$X), seed = 9)
  expect_true(is.finite(mse1))
  
  mse_all <- proper_cv_2fold(dat, p = ncol(dat$X), K = ncol(dat$X),
                             sample_size = nrow(dat$X), seed = 9)
...
test_that("reproducible for identical seeds, different for different seeds", {
  ...
  a1 <- proper_cv_2fold(dat, p = ncol(dat$X), K = 3,
                        sample_size = nrow(dat$X), seed = 82803)
  a2 <- proper_cv_2fold(dat, p = ncol(dat$X), K = 3,
                        sample_size = nrow(dat$X), seed = 82803)
  expect_identical(a1, a2)
  
  b1 <- proper_cv_2fold(dat, p = ncol(dat$X), K = 3,
...
test_that("input validation: y/X length mismatch errors", {
  ...
  expect_error(
    proper_cv_2fold(bad, p = ncol(sample$X), K = 3, sample_size = nrow(sample$X), seed = 1),
